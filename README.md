...
Создайте файлы `.env`, скопировав готовые шаблоны, и подставьте собственные значения:
...
| `SECRET_KEY`    | Да                      | Секрет для подписи JWT. Сгенерируйте длинную случайную строку (`openssl rand -hex 32`).          |
| `FROM_EMAIL`    | Нет                     | Email, который увидит получатель. Если оставить пустым, будет использовано значение `SMTP_USER`. |
| `SMTP_HOST`     | Да (для отправки писем) | Домен или IP SMTP сервера.                                                                       |
| `SMTP_PORT`     | Да                      | Порт SMTP. Для STARTTLS чаще всего `587`.                                                        |
| `SMTP_USER`     | Нет                     | Логин для SMTP. Укажите, если сервер требует авторизации.                                        |
| `SMTP_PASSWORD` | Нет                     | Пароль или токен для SMTP.                                                                       |
| `SMTP_USE_TLS`  | Нет                     | `true/false` — включить STARTTLS. Для большинства провайдеров оставьте `true`.                   |
| `SMTP_TIMEOUT`  | Нет                     | Таймаут соединения в секундах (по умолчанию 30).                                                 |

> Если не задать `SMTP_HOST`, сервис просто пропустит отправку письма и вернет `email_sent: false` — это удобно для разработки без почты.
...
3. Установите `SMTP_HOST=smtp.gmail.com`, `SMTP_PORT=587`, `SMTP_USE_TLS=true`, `SMTP_USER=<ваш_gmail>@gmail.com`, `FROM_EMAIL=<ваш_gmail>@gmail.com`.
