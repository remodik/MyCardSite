...
|-----------------------|-------------------------|-------------------------------------------------------------------------------------------------------|
| `DATABASE_URL`        | Нет                     | Строка подключения SQLAlchemy. По умолчанию используется `sqlite+aiosqlite:///./backend/projects.db`. |
...
- `admin_reset_requests` — Запросы на сброс паролей админом (id, user_id, username, status, requested_at, completed_at)
...
# Просмотр содержимого SQLite (пример)
sqlite3 backend/projects.db ".tables"
```
